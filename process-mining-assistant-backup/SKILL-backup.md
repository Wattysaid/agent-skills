...
description: Perform an end‑to‑end process mining analysis with interactive decision gates, step‑wise execution, and per‑stage notebooks/artifacts using PM4Py. Use this skill when the user wants you to set up a virtual environment, run scripts to ingest and analyze event logs, then iterate through discovery, conformance, and reporting based on user choices and notebook validation.
---

## Overview

This skill equips you with a repeatable, step‑wise workflow for conducting comprehensive process mining analyses with decision gates and per‑stage notebooks. It is designed for business analysts and data scientists who need to run the entire process mining lifecycle—from data ingestion through discovery, conformance checking and performance analysis—while reviewing artifacts and notebooks at each stage. The skill assumes familiarity with Python and the PM4Py library.
...
### Interactive Execution Loop (Required)

Use this loop for every project, in order:

1. **Kickoff**: user asks a question or starts the work. Ask only the first phase questions needed to proceed (typically environment + input file discovery).
2. **Environment setup**: create a project-local virtual environment (preferred) and install dependencies from `requirements.txt` unless the user chooses otherwise.
3. **Validate**: run `scripts/00_validate_env.py` in the active venv before any analysis.
4. **Ingest**: run `scripts/01_ingest.py` and store outputs in a dedicated stage folder.
5. **Review gate**: summarize initial findings from ingest artifacts, provide paths to outputs and the stage notebook, and ask the next phase questions. Do not proceed until the user answers the numbered choices.
6. **Sequential steps**: run the next step scripts (data quality, filtering, EDA, discovery, conformance, performance, org mining, reporting) in order, with a review gate after each step.
7. **Notebook check**: before every step after ingest, verify whether the user edited the previous stage notebook. If edits exist, acknowledge them, re-load or re-run as needed, and confirm whether to proceed.

### Notebook & Artifact Lifecycle (Required)

- Create a dedicated run folder and a subfolder per stage. Use a stable pattern such as `runs/YYYYMMDD-HHMMSS/01_ingest`, `runs/YYYYMMDD-HHMMSS/02_data_quality`, etc.
- Each stage must include:
  - output artifacts produced by the script (CSV, JSON, PNG, etc.)
  - a stage notebook (e.g., `notebooks/01_ingest_v001.ipynb`) that documents the inputs, code used, and key outputs
  - a small `manifest.json` capturing the stage parameters, artifact paths, and notebook file hash
- At every review gate, tell the user where these artifacts and the notebook live so they can inspect or edit before choosing the next option.
- **Versioning rule**: when you re-run a stage or after the user edits a notebook, increment the notebook version (v002, v003, ...). Keep all prior versions to preserve a progressive trail.
- **Notebook change detection**: before moving to the next stage, compare the current notebook hash (or mtime) to the hash recorded in the stage manifest. If changed, re-open the notebook, summarize the user’s edits, and confirm whether to incorporate them before proceeding.
...
The workflow accepts event logs in either **XES** or **CSV** format and outputs reports, visualisations and diagnostic metrics. It follows best practices for modularity and reproducibility, mirroring the notebook‑based process mining guidance while retaining scriptable execution and review gates after each stage.
...
1. **Installation + Environment**
   ...
   - Install dependencies using pip:
     ...
     pip install pm4py pandas numpy matplotlib
     ```
   - Create a project-local virtual environment (preferred) and activate it.
   - Install dependencies from `requirements.txt`.
   - Run `python scripts/00_validate_env.py` to confirm the environment.

2. **Prepare the Stage Directories**

   - Create a run folder for the session and a subfolder per stage (ingest, data quality, filtering, EDA, discovery, conformance, performance, organisational mining, reporting).
   ...
   - Each script focuses on a single phase and shares common utilities in `scripts/common.py`.
   - For every stage, produce a notebook that documents inputs, code, and outputs, and save a manifest that tracks parameters and notebook version/hash.
