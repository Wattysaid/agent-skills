# Camunda 8 BPMN Output Guide

Use this when the user requests Camunda 8 output or "Camunda 8 BPMN".

Start with `Instructions/00b_README_MASTER.md` and `Instructions/01_authoring_contract.md`.

Use `references/` in this folder for Camunda 8-specific overrides; generic BPMN rules live in the root `references/`.
Start with `references/syntax_overrides.md` for Camunda 8 output constraints.

## Target Output

- BPMN 2.0 XML compatible with Camunda 8.
...
- Only add Camunda extensions if explicitly requested or provided.
...
- Required extensions (forms, workers, jobs)?
